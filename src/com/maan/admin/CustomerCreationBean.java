...
public class CustomerCreationBean extends ErrorInfo 
{
	...
	public String updateCusLogStatus(String accode,String process,String sdate)
	{
		...
		try
		{
			...
			{
				String sql2 = "update Login_Master set EXPIRY_DATE='"+com.maan.common.util.OracleDateConversion.ConvertDate(""+sdate)+"',ENTRY_DATE=(select sysdate from dual) where agency_code in('"+accode+"')";
				res = com.maan.services.util.runner.updation(sql2);
	...
	public String[][] getSelected(String loginId, String appId, String status) 
	{
		...
		try
		{
			...
			if(!status.equalsIgnoreCase("E"))
			{
				...
				if(status.equalsIgnoreCase("DD"))
				{
					sql = "select per.CUSTOMER_ID,per.APPLICATION_ID,nvl(per.FIRST_NAME,per.company_name),per.agency_code,to_char(log.EXPIRY_DATE,'dd-mm-yyyy') from personal_info per, login_master log where  per.APPLICATION_ID=? and per.oa_code=? and per.agency_code=log.agency_code and log.EXPIRY_DATE>(select sysdate from dual) and per.status='Y'";
					String args[] = new String[2];
					args[0] = appId;
					args[1] = oacode;
					brokers = runner.multipleSelection(sql,args);
			...
			{
				sql = "select per.CUSTOMER_ID,per.APPLICATION_ID,nvl(per.FIRST_NAME,per.company_name),per.agency_code,to_char(log.EXPIRY_DATE,'dd-mm-yyyy') from personal_info per, login_master log where  per.APPLICATION_ID=? and per.oa_code=? and per.agency_code=log.agency_code and log.EXPIRY_DATE<=(select sysdate from dual)";
				String args[] = new String[2];
				args[0] = appId;
				args[1] = oacode;
				brokers = runner.multipleSelection(sql,args);
	...
	{
		String sql = "select count(*) from LOGIN_USER_DETAILS where oa_code=(select oa_code from login_master where login_id=?) and CUSTOMER_CERTIFICATE='Y' and product_id='11' order by product_id";
