...
{
	private static final Logger logger = LogUtil.getLogger(BrokerCreationBean.class);
	...
	public String insertBrokerEntry() 
	{
		...
		if (customerid <= 0)
		{
			...
			String sqlboth = "insert into broker_company_master(AGENCY_CODE,COMPANY_NAME,CONTACT_PERSON,ADDRESS1,ADDRESS2,ADDRESS3,CITY,COUNTRY,PHONE,POBOX,FAX,EMIRATE,REMARKS,STATUS,CUSTOMER_ID,BRANCH_CODE,MISSIPPI_ID,APPROVED_PREPARED_BY,rsa_broker_code) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		else 
		{
			...
			String sqlboth = "update  broker_company_master set COMPANY_NAME=?,CONTACT_PERSON=?,ADDRESS1=?,ADDRESS2=?,ADDRESS3=?,CITY=?,COUNTRY=?,PHONE=?,POBOX=?,FAX=?,EMIRATE=?,CUSTOMER_ID=?,BRANCH_CODE=?,MISSIPPI_ID=?,APPROVED_PREPARED_BY=?,RSA_BROKER_CODE=? where agency_code=? and branch_code=?";
		...
		if (!checkCustomerId(String.valueOf(customerid))) 
		{
			try
			{
				...
				String qry = "insert into personal_info(customer_id,application_id,title,first_name,last_name,amend_id,nationality,dob,gender,telephone,mobile,fax,email,address1,address2,occupation,pobox,country,emirate,status,entry_date,login_id,agency_code,oa_code)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		...
		else
		{
			...
			try
			{
				String qry = "update personal_info set title=?,first_name=?,last_name=?,nationality=?,dob=?,gender=?,telephone=?,mobile=?,fax=?,email=?,address1=?,address2=?,occupation=?,pobox=?,country=?,emirate=?,status=?,effective_date=?  where application_id='2' and agency_code=?";
	...
	public String insertOrUpdate(String bcode,String loginPersonId,String branchCode) 
	{
		java.util.Date dd = new java.util.Date();
		java.text.SimpleDateFormat simpleFormatter = new java.text.SimpleDateFormat("dd-MM-yyyy");
		simpleFormatter.setTimeZone(new java.util.SimpleTimeZone(14400000,"GMT"));            	
			...
			for (i = 0; i < proDetails.size(); i++) 
			{
					...
					if (userdetails.length > 0)
					{
						...
						else 
						{
							...
							try
							{
								...
								else
								{
									...
									logger.info("Product Id-->"+office);
									logger.info("Debit Option-->"+debit);
									args[8] = debit;
									args[9] = loadDis;
									args[10] = bcode;
									args[11] = office;//(String) pro.get("product" + (i + 1));
						...
						if (!checkBrokerCode(bcode, "login_master", ""))
						{
							...
							String sql1 = "insert into login_master(LOGIN_ID,PASSWORD,USERTYPE,USERNAME,USERID,AGENCY_CODE,OA_CODE,ACCESSTYPE,RIGHTS,LPASS1,LPASS2,LPASS3,PASSDATE,COMPANY_ID,CREATED_BY,STATUS,USER_ID_CREATION,AC_EXECUTIVE_CREATION,REFERAL,BRANCH_CODE) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public boolean changePassword(String brokerId, String password) 
	{
		...
		String encpass = pass.crypt(password.substring(0, 3), password);
		boolean check = false;
	...
	public String[][] getRSAAdmins(String branch,String type)
	{
		...
		try 
		{
			...
			{
				sql = "select usertype,login_id,username,agency_code from login_master where usertype in('RSAIssuer') and branch_code=? order by username";
				brokers = runner.multipleSelection(sql,args);
			}
			else
			{
				...
				if(type.equalsIgnoreCase("UWAdmin")||type.equalsIgnoreCase("RSAUSER")||type.equalsIgnoreCase("admin"))
					adminType = "'UWAdmin','RSAUSER','admin','ClaimUser'";
				else
					adminType = "'HomeRSAUser','HomeAdmin'";
				sql = "select usertype,login_id,username,agency_code from login_master where usertype in("+adminType+") and branch_code=? order by username";
				brokers = runner.multipleSelection(sql,args);
			}
...
//		 Exchange master Effective Date Validation
	...
	public String insertExchangeData() 
	{
		...
		try
		{
			sql = "insert into exchange_master(SNO__,EXCHANGE_ID,EXCHANGE_RATE,CURRENCY_ID,AMEND_ID,INCEPTION_DATE,EXPIRY_DATE,EFFECTIVE_DATE,ENTRY_DATE,STATUS,REMARKS,RSACODE,COUNTRY_ID,DISPLAY_ORDER) values(?,?,?,?,?,(select "+temp+" from dual),?,TO_DATE(?,'dd/mon/yyyy HH24:MI:SS')+10/24,(select "+temp+" from dual),?,?,?,?,?)";
	...
	public String insertCurrencyData() 
	{
		...
		try 
		{
			...
			sql = "insert into currency_master(SNO__,CURRENCY_ID,CURRENCY_NAME,AMEND_ID,INCEPTION_DATE,EFFECTIVE_DATE,STATUS,REMARKS,country_id,display_order) values(?,?,?,?,(select "+temp+" from dual),TO_DATE(?,'dd/mon/yyyy HH24:MI:SS')+10/24,?,?,?,?)";
	...
	public String getMaxCustomerId(String branch) 
	{
		...
		try
		{
			...
			/*sql="SELECT SEQ_NAME FROM SEQUENCE_MASTER WHERE SEQ_ID='1' AND  BRANCH_CODE=? AND PRODUCT_ID=33";
			seqName = runner.singleSelection(sql,args);
	...
	public String[][] getExchangeDatas(String crid) 
	{
		...
		try 
		{
			...
			args[3] = crid;
			sql = "select sno__ from exchange_master where amend_id=(select max(amend_id) from exchange_master where currency_id=? and country_id=? and effective_date<=(select "+temp+" from dual)) and country_id=? and currency_id=?";
			res = runner.singleSelection(sql,args);
			if(res.length() > 0 && ! res.equalsIgnoreCase("null")) 
			{
				sno = res;
	...
	public String[][] getCurrencyData(String crid,String countryId)
	{
		...
		try 
		{
			...
			args[2] = crid;
			args[3] = countryId;
			sql = "select sno__  from currency_master where amend_id=(select max(amend_id) from currency_master where currency_id=? and country_id=? and effective_date<=(select "+temp+" from dual) ) and currency_id=? and country_id=?";
			res = runner.singleSelection(sql,args);		
			if(res.length() > 0 && !res.equalsIgnoreCase("null")) 
			{
				sno = res;
		...
		try 
		{
			...
			args[0] = sno;
			args[1] = countryId;
			sql = "select sno__,currency_id,CURRENCY_NAME,amend_id,to_char(EFFECTIVE_DATE,'dd-mm-yyyy'),remarks,status,rsacode from currency_master where sno__=? and country_id=?";
...
/** UserCreationController.jsp **/
	public String validateNewAdmin(String mode,String userCategory,String emailOption)
	{
		...
		if ((user_Name.equalsIgnoreCase("")) || (user_Name == null))
			error = error + "<br>* Please eneter User Name";
		if ((login_Id.equalsIgnoreCase("")) || (login_Id == null) || login_Id.indexOf(" ")!=-1 ){
			error = error + "<br>* Please eneter Login Id";
		}
		else 
		{
		...
		/*try
		{
			if(!User_prodType.equalsIgnoreCase(userCategory))
				 error = error + "<br>* Please Select Valid Admin Product Type";
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	public boolean performInsertion(String mode) 
	{
		...
		if(!mode.equalsIgnoreCase("edit"))
		{
			try
			{
				sql1 = "insert into login_master(LOGIN_ID,PASSWORD,USERTYPE,USERNAME,USERID,AGENCY_CODE,OA_CODE,COMPANY_ID,CREATED_BY,STATUS,USER_ID_CREATION,AC_EXECUTIVE_CREATION,ACCESSTYPE,PASSDATE,BRANCH_CODE,COUNTRY_ID,MENU_ID,PRODUCT_ID,broker_codes) values(?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?,?,?,?,?)";
				
				passwordEnc pass = new passwordEnc();
				String encpass = pass.crypt(user_Pass.substring(0, 3), user_Pass);
				boolean check = false;
				args = new String[18];
				args[0] = login_Id;
				args[1] = encpass;
	...
	public String insertBankMasterData()
	{
		...
		{
			args = new String[8];
			sql = "insert into OPEN_COVER_BANK_MASTER(BANK_ID,BANK_NAME,ENTRY_DATE,EXPIRY_DATE,EFFECTIVE_DATE,AMEND_ID,REMARKS,STATUS, RSACODE,BELONGING_COUNTRY_ID) values(?,?,(select "+temp+" from dual),(select "+temp+" from dual),TO_DATE(?,'dd/mon/yyyy')+10/24,?,?,?,?,?)";
	...
	public String[][] getBrokerInDetails(String acode, String type)
	{
		...
		try
		{
			...
			else if(type.equalsIgnoreCase("User"))//type as User
			{
				...
				args[0] = type;
				args[1] = acode;
				sql = "select nvl(per.first_name,per.company_name),per.status,log.usertype,per.application_id,per.agency_code from personal_info per,login_master log where log.usertype=? and log.oa_code=? and log.agency_code=per.agency_code AND LOG.LOGIN_ID=PER.LOGIN_ID and log.login_id not in('NONE') and per.application_id='3'";
				res = runner.multipleSelection(sql,args);
			}
	...
	public String[][] getBrokerInName(String acode, String type)
	{
		...
		try
		{
			...
			else
			{
				...
				args[0] = type;
				args[1] = acode;
				sql = "select nvl(nvl(per.first_name,'')+''+nvl(per.last_name,''),nvl(per.company_name,'')),per.status,log.usertype,per.application_id,per.agency_code from personal_info per,login_master log where log.usertype=? and per.agency_code=? and log.agency_code=per.agency_code and log.login_id not in('NONE') and APPLICATION_ID!=1";
				res = runner.multipleSelection(sql,args);
			}
		}
		catch(Exception e)
	...
	public String[][] getAdminMenu(String branch,String prodType)
	{
		...
		try
		{
			...
			{
				sql = "select CATEGORY_DETAIL_ID,DETAIL_NAME,REMARKS from CONSTANT_DETAIL where status='Y' and CATEGORY_ID='41' and branch_code=? and CATEGORY_DETAIL_ID in('1','2','8','17','18','19','20','21','24','26','16','27','28','33','43','11','31') order by CATEGORY_DETAIL_ID"; 
			}
	...
	public String getAdminUserType(String acode)
	{
		...
		{
			args[0] = acode;
			sql = "select usertype from login_master where agency_code=?";
			res = runner.singleSelection(sql,args);
		}
		catch(Exception e)
		{
	...
	{
		String sql = "select broker_code,product_id,commission,DATEPART(day, START_DATE),DATEPART(month, START_DATE),DATEPART(year, START_DATE),DATEPART(day, END_DATE),DATEPART(month, END_DATE),DATEPART(year, END_DATE),status,DATEPART(day, CANCEL_DATE),DATEPART(month, CANCEL_DATE),DATEPART(year, CANCEL_DATE) from LOGIN_RSAUSER_DETAILS where login_id=? and (amend_id+broker_code+product_id) in(select max(amend_id)+broker_code+product_id from LOGIN_RSAUSER_DETAILS where login_id=? group by broker_code,product_id) order by broker_Code,product_id";
		String args[] = new String[2];
		String result[][]=new String[0][0];
		try
		{
		...
		try
		{
			...
			if(	commission.length()>0)
			{
				...
				else if("Invalid".equalsIgnoreCase(values1)&&!activeStaus.equalsIgnoreCase("N")) 
				{
                      ...
                      c1.set(Integer.parseInt(dobYear),Integer.parseInt(dobMonth),Integer.parseInt(dobDay));
				...
				if(mode.equalsIgnoreCase("edit")&&"Invalid".equalsIgnoreCase(values1)&&!activeStaus.equalsIgnoreCase("N"))
				{
					try
					{
						...
						args[4] = brokerIds;
						args[5] = pid;
						lastendDate = runner.singleSelection("select to_char(end_Date,'dd/mm/yyyy') from LOGIN_RSAUSER_DETAILS where login_id=? and START_DATE<=sysdate and END_DATE>=sysdate) and amend_id=(select max(amend_id) from LOGIN_RSAUSER_DETAILS where login_id=? and START_DATE<=sysdate) and END_DATE>=sysdate)  and broker_code=? and product_id=?) and broker_code=? and status='Y' and product_id=?",args);
				...
				else if(activeStaus.equalsIgnoreCase("N")&&"Valid".equalsIgnoreCase(values))
				{
					com.maan.Home.DataManipualtion.DataSelect 	dataSelect	=	new com.maan.Home.DataManipualtion.DataSelect();
					  Calendar c1 = Calendar.getInstance();
                      c1.set(Integer.parseInt(dobYear2),Integer.parseInt(dobMonth2),Integer.parseInt(dobDay2));
                      Calendar c2 = Calendar.getInstance();
				      String[][] serverDate =  dataSelect.getTodaysDate();
			           c2.set(Integer.parseInt(serverDate[0][0]),Integer.parseInt(serverDate[0][1]),Integer.parseInt(serverDate[0][2]));
                      diff = 0;
					...
					if(diff>=0)
					{
					 ...
					 c2.set(Integer.parseInt(dobYear1),Integer.parseInt(dobMonth1),Integer.parseInt(dobDay1));
					  diff = 0;
                      try
					  {
                           diff = dbr.getDayDifference(c1,c2);
	...
	public void insertRSAIssuer(String mode) 
	{
		...
		if(!mode.equalsIgnoreCase("edit"))
		{
			try
			{
				sql1 = "insert into login_master(LOGIN_ID,PASSWORD,USERTYPE,USERNAME,USERID,AGENCY_CODE,OA_CODE,COMPANY_ID,CREATED_BY,STATUS,USER_ID_CREATION,AC_EXECUTIVE_CREATION,ACCESSTYPE,PASSDATE,BRANCH_CODE,COUNTRY_ID) values(?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?,?)";
			...
			try
			{
				String count1 = runner.singleSelection("select count(*) from login_user_details where LOGIN_ID='"+login_Id+"' and product_id=33");
				String count2 = runner.singleSelection("select count(*) from login_user_details where LOGIN_ID='"+login_Id+"' and product_id=41");
	...
	public void insertRSAIssuerDetails()
	{
		...
		if(activeStaus.equalsIgnoreCase("Y"))
		{
			...
			sql = "insert into LOGIN_RSAUSER_DETAILS(LOGIN_ID,BROKER_CODE,PRODUCT_ID,AMEND_ID,BRANCH_CODE,START_DATE,END_DATE,ENTRY_DATE,COMMISSION,STATUS,OPEN_COVER_NO)  values(?,?,?,?,?,CONVERT(DATETIME,?,'dd/mm/yyyy'),CONVERT(DATETIME,?,'dd/mm/yyyy'),sysdate,?,'Y',?)";
			try
			{
		...
		{
			sql = "update LOGIN_RSAUSER_DETAILS set status='N',CANCEL_DATE=CONVERT(DATETIME,?,'dd/mm/yyyy'),END_DATE=dateadd(day,-1,CONVERT(DATETIME,?,'dd/mm/yyyy')),ENTRY_DATE=sysdate where login_id=? and broker_code=? and PRODUCT_ID=? and BRANCH_CODE=? and amend_id=?";
			try
			{
	...
	public String getOpenCoverCertificatesUser(String loginId,String broCode)
	{
		...
		try
		{
			...
			args[2] = broCode;
			args[3] = broCode;
			String  opensql = "select OPEN_COVER_NO from LOGIN_RSAUSER_DETAILS where login_id=? and product_id='11' and START_DATE<=(select to_char("+temp+",'dd-mon-yyyy') from dual) and END_DATE>=(select to_char("+temp+",'dd-mon-yyyy') from dual) and amend_id=(select max(amend_id) from LOGIN_RSAUSER_DETAILS where login_id=? and product_id='11' and START_DATE<=(select to_char("+temp+",'dd-mon-yyyy') from dual) and END_DATE>=(select to_char("+temp+",'dd-mon-yyyy') from dual) and broker_code=?) and broker_code=? and status='Y'";
	...
	public String[][] getBrokerTaxInfo(final String branchCode,final String aCode){
		logger.info("entered getBrokerTaxInfo()");
		String[][] result = new String[0][0];
		try{
			String args[] = {branchCode,aCode,branchCode,aCode};
			String sql;sql = "select nvl(POLICY_FEE_STATUS,'N'),nvl(POLICY_FEE,'0'),nvl(GOVT_TAX_STATUS,'N'),nvl(GOVT_TAX,'0'),nvl(EMERGENCY_FUND_STATUS,'N'),nvl(EMERGENCY_FUND,'0'),to_char(EFFECTIVE_DATE,'dd/mm/yyyy') " +
					"from BROKER_MASTER " +
		...
		logger.info("exit getBrokerTaxInfo()");
		return result;
	}
	public void brokerMasterInsert(final Map broDetails){
		logger.info("entered brokerMasterInsert()");
		try{
			...
			String sql;sql = "insert into BROKER_MASTER(AGENCY_CODE,BRANCH_CODE,POLICY_FEE_STATUS,GOVT_TAX_STATUS,EMERGENCY_FUND_STATUS," +
					"POLICY_FEE,GOVT_TAX,EMERGENCY_FUND,STATUS,REMARKS,AMEND_ID,ENTRY_DATE,EFFECTIVE_DATE)" +
					" values(?,?,?,?,?,?,?,?,'Y',?,?,sysdate,convert(datetime,?,'dd/mm/yyyy'))";
			String cols[] = {(String)broDetails.get("bcode"),(String)broDetails.get("branchCode"),
					...
					("N".equals(broDetails.get("feeStatus").toString())?"0":(String)broDetails.get("policFee")),("N".equals(broDetails.get("taxStatus").toString())?"0":(String)broDetails.get("govtTax")),("N".equals(broDetails.get("fundStatus").toString())?"0":(String)broDetails.get("emergencyfund"))
					,(String)broDetails.get("loginId"),Integer.toString((Integer.parseInt(amendId)+1)),(String)broDetails.get("effectiveDate")};
			runner.multipleInsertion(sql, cols);
		}catch(Exception e){
			e.printStackTrace();
		}
		logger.info("exit brokerMasterInsert()");
	}
	public String getBrokerMaxAmend(final Map broDetails){
		logger.info("entered getBrokerMaxAmend()");
		String amendId="";
		try{
		...
		logger.info("exit getBrokerMaxAmend()");
		return amendId;
	}
	public void updateBrokerMaster(Map broDetails,String amendId){
		logger.info("entered updateBrokerMaster()");
		try{
			String sql;sql="update BROKER_MASTER set STATUS='N',INCEPTION_DATE=sysdate,EXPIRY_DATE=convert(datetime,?,'dd/mm/yyyy')" +
		...
		logger.info("exit updateBrokerMaster()");
	}
	public String brokerMasterValidation(final Map broDetails){
		logger.info("entered brokerMasterValidation()");
	...
	public boolean isBrokerMasterEffectiveValid(final Map broDetails,String amednId){
		...
		logger.info("exit isBrokerMasterEffectiveValid()");
		return flag;
	}
	...
	//Added by Chinna
	public String [][] getPolicyTypeDetails(String branchCode, String proId, String loginId)
	{
		System.out.println("getPolicyTypeDetails - Enter");
		String q="SELECT Y.POLICYTYPE_ID, Y.POLICYTYPE_DESC_ENGLISH, Y.POLICYTYPE_DESC_ARABIC, X.START_RATE*100, X.END_RATE*100, X.PROVISION_FOR_RATE, X.STATUS, to_char(X.EFFECTIVE_DATE,'dd-mm-yyyy'), to_char(X.EXPIRY_DATE,'dd-mm-yyyy') FROM (SELECT a.POLICYTYPE_ID, b.START_RATE, b.END_RATE, b.PROVISION_FOR_RATE, b.STATUS, b.EFFECTIVE_DATE, b.EXPIRY_DATE FROM MOTOR_POLICYTYPE_MASTER a, MOTOR_BROKER_POLICYTYPE_MASTER b WHERE b.POLICYTYPE_IDS = a.POLICYTYPE_ID AND b.BRANCH_CODE = a.BRANCH_CODE AND a.BRANCH_CODE = ? AND b.PRODUCT_ID = ? AND b.BROKER_ID=? AND a.AMEND_ID = (SELECT MAX (AMEND_ID) FROM MOTOR_POLICYTYPE_MASTER WHERE POLICYTYPE_ID = a.POLICYTYPE_ID AND BRANCH_CODE = a.BRANCH_CODE) AND b.AMEND_ID = (SELECT MAX (AMEND_ID) FROM MOTOR_BROKER_POLICYTYPE_MASTER WHERE POLICYTYPE_IDS = a.POLICYTYPE_ID AND BRANCH_CODE = a.BRANCH_CODE AND PRODUCT_ID = b.PRODUCT_ID AND BROKER_ID=b.BROKER_ID)) X RIGHT JOIN (SELECT POLICYTYPE_ID, POLICYTYPE_DESC_ENGLISH, POLICYTYPE_DESC_ARABIC FROM MOTOR_POLICYTYPE_MASTER WHERE BRANCH_CODE = ?) Y ON (X.POLICYTYPE_ID = Y.POLICYTYPE_ID) ORDER BY Y.POLICYTYPE_ID";
		String args[]=new String[4];
		args[0]=branchCode;
		args[1]=proId;
		args[2]=loginId;
		...
		System.out.println("getPolicyTypeDetails - Exit || Result Length: "+result.length);
		return result;
	}
	public String getLoginId(String brokerId, String proId)
	{
		System.out.println("getLoginId - Enter");
		String q="select LOGIN_ID from LOGIN_USER_DETAILS where AGENCY_CODE=? AND PRODUCT_ID=? and AMEND_ID=(SELECT MAX(AMEND_ID) FROM LOGIN_USER_DETAILS where AGENCY_CODE=? AND PRODUCT_ID=? )";
		String args[]=new String[4];
		args[0]=brokerId;
		args[1]=proId;
		args[2]=brokerId;
		args[3]=proId;
		String loginId=runner.singleSelection(q, args);
		System.out.println("getLoginId - Exit || Result: "+loginId);
		return loginId;
	}
	public String updateLoginId(String loginId, String branchCode,String brokerId)
	{
		System.out.println("updateLoginId - Enter");
		String q="UPDATE  MOTOR_BROKER_POLICYTYPE_MASTER SET BROKER_ID=? where BROKER_ID=? AND BRANCH_CODE=?";
		String args[]=new String[3];
		args[0]=loginId;
		args[1]=brokerId;
		args[2]=branchCode;
		String result="UPDATED";//runner.multipleUpdation(q, args);
		System.out.println("updateLoginId - Exit || Result: "+result);
		return result;
	}
	public String getMaxAmendId(String proId, String branchCode, String brokerId)
	{
		System.out.println("getMaxAmendId - Enter");
		String q="";
		String result="";
		try
		{
		q="DELETE FROM MOTOR_BROKER_POLICYTYPE_MASTER WHERE PRODUCT_ID='"+proId+"' AND BRANCH_CODE='"+branchCode+"' AND BROKER_ID='"+brokerId+"' AND AMEND_ID=(SELECT MAX(AMEND_ID) FROM MOTOR_BROKER_POLICYTYPE_MASTER WHERE PRODUCT_ID='"+proId+"' AND BRANCH_CODE='"+branchCode+"' AND BROKER_ID='"+brokerId+"')";
		runner.deletion(q);
		System.out.println("Delete Query: "+q);
		String args[]=new String[3];
//		String q="SELECT (case when count(*)=0 then 0 else max(amend_id)+1 end) FROM MOTOR_BROKER_POLICYTYPE_MASTER WHERE PRODUCT_ID=? AND BRANCH_CODE=? AND BROKER_ID=?";
		q="SELECT (case when count(*)=0 then 0 else max(amend_id) end) FROM MOTOR_BROKER_POLICYTYPE_MASTER WHERE PRODUCT_ID=? AND BRANCH_CODE=? AND BROKER_ID=?";
		args=new String[3];
		args[0]=proId;
		args[1]=branchCode;
		args[2]=brokerId;
		...
		System.out.println("getMaxAmendId - Exit || Result: "+result);
		return result;
	}
	public void insertPolicyTypeDetails(String proId, String branchCode, String brokerId, String amendId, String values)
	{
		System.out.println("insertPolicyTypeDetails - Enter");
		String q="";
		String result="";
		try{
		q="INSERT INTO MOTOR_BROKER_POLICYTYPE_MASTER(AMEND_ID,PRODUCT_ID,BRANCH_CODE,BROKER_ID,POLICYTYPE_IDS,START_RATE,END_RATE,PROVISION_FOR_RATE,STATUS,EFFECTIVE_DATE) VALUES(?,?,?,? "+values+")";
		String[] args=new String[4];
		args[0]=amendId;
		args[1]=proId;
		args[2]=branchCode;
		args[3]=brokerId;
		result=runner.multipleInsertion(q, args);
		}catch(Exception e)
		{
			e.printStackTrace();
		}
		System.out.println("Query: "+q);
		System.out.println("insertPolicyTypeDetails - Exit || Result: "+result);
	}
	public void insertUserPolicyTypeDetails(String proId, String branchCode, String brokerId, String amendId, String values)
	{
		System.out.println("insertPolicyTypeDetails - Enter");
		String q="";
		String result="";
		try{
		q="INSERT INTO MOTOR_BROKER_POLICYTYPE_MASTER(AMEND_ID,PRODUCT_ID,BRANCH_CODE,BROKER_ID,POLICYTYPE_IDS,STATUS) VALUES(?,?,?,? "+values+")";
		String[] args=new String[4];
		args[0]=amendId;
		args[1]=proId;
		args[2]=branchCode;
	...
	public String [][] getUserPolicyTypeDetails(String branchCode, String proId, String loginId, String userloginId)
	{
		System.out.println("getUserPolicyTypeDetails - Enter");
		String q="SELECT a.POLICYTYPE_IDS,b.POLICYTYPE_DESC_ENGLISH, a.status FROM MOTOR_BROKER_POLICYTYPE_MASTER a, MOTOR_POLICYTYPE_MASTER b WHERE a.BRANCH_CODE = ? AND a.PRODUCT_ID = ? AND a.BROKER_ID = ? AND b.POLICYTYPE_ID = a.POLICYTYPE_IDS AND b.BRANCH_CODE = a.BRANCH_CODE UNION SELECT a.POLICYTYPE_IDS,b.POLICYTYPE_DESC_ENGLISH, '' FROM MOTOR_BROKER_POLICYTYPE_MASTER a, MOTOR_POLICYTYPE_MASTER b WHERE a.BRANCH_CODE = ? AND a.PRODUCT_ID = ? AND a.BROKER_ID = ? AND b.POLICYTYPE_ID = a.POLICYTYPE_IDS AND b.BRANCH_CODE = a.BRANCH_CODE AND a.POLICYTYPE_IDS not in (SELECT POLICYTYPE_IDS FROM MOTOR_BROKER_POLICYTYPE_MASTER WHERE BRANCH_CODE = ? AND PRODUCT_ID = ? AND BROKER_ID = ?)";
		String args[]=new String[9];
		args[0]=branchCode;
		args[1]=proId;
		args[2]=userloginId;
		...
		args[8]=userloginId;
		String result[][] = runner.multipleSelection(q,args);
		System.out.println("getUserPolicyTypeDetails - Exit || Result Length: "+result.length);
		return result;
	}
