	...
	/*public String insertCloseTRN(String loginBra,String cid)
	{
		...
		public String insertCloseTRN(String loginBra,final String cid,final String productCode)throws BaseException
		{
			...
			if(trn_close.length>0)
			{
				...
				args[4] = loginBra;
				args[5] = productCode;
				sql = "update T_TRN_CLOSING set CLO_DATE_CLOSED=to_date(?,'DD-MM-YYYY'),CLO_REMARKS=?,CLO_PREPARED_DT=to_date( ?,'DD-MM-YYYY'),CLO_DATE_OPENED=to_date(?,'DD-MM-YYYY') where CLO_TRAN_CODE=1 and BRANCH_CODE=? and PRODUCT_CORE_CODE=?";
			...
			else
			{
				...
				args[4] = loginBra;
				args[5] = productCode;
				sql = "insert into T_TRN_CLOSING(CLO_TRAN_CODE,CLO_DATE_CLOSED,CLO_REMARKS,CLO_PREPARED_DT,CLO_DATE_OPENED,BRANCH_CODE,PRODUCT_CORE_CODE) values(1,to_date(?,'DD-MM-YYYY'),?,to_date( ?,'DD-MM-YYYY'),to_date(?,'DD-MM-YYYY'),?,?)";
			...
			args[6] = ClosedRemarks;
			args[7] = "Y";
			args[8] = loginBra;
			args[9]= cid;
			args[10]= productCode;
			sql = "insert into TRN_CLOSING_DETAILS values(?, ?, to_date(?,'DD-MM-YYYY'), to_date(?,'DD-MM-YYYY'), to_date(?,'DD-MM-YYYY'), " +
