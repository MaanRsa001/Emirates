...
public class UserCreationBean extends ErrorInfo 
{
	...
	public String insertBrokerEntry(String branch,String b2c) 
	{
		String user_Id = "1";
		String Query = "";
		...
		try 
		{
			...
			if ("0".equalsIgnoreCase(qry)|| "DIDN'T SELECTED".equalsIgnoreCase(qry)) 
			{
				...
				{
				    String query="INSERT INTO LOGIN_USER_DETAILS(USER_ID,USER_NAME,login_id,AGENCY_CODE,CUSTOMER_ID,OA_CODE,PRODUCT_ID,COMPANY_ID,COMMISSION,INSURANCE_START_LIMIT,INSURANCE_END_LIMIT,SPECIAL_DISCOUNT,RELATIVE_USER_ID,AMEND_ID,INCEPTION_DATE,ENTRY_DATE,STATUS,MIN_PREMIUM_AMOUNT, BACK_DATE_ALLOWED,LOADING_OF_PREMIUM,PROVISION_FOR_PREMIUM,DISCOUNT_OF_PREMIUM,FREIGHT_DEBIT_OPTION,PRO_COMMISSION)SELECT  ( select (max(USER_ID)+1) from LOGIN_USER_DETAILS) nm,?,'none',?,?,OA_CODE,PRODUCT_ID,COMPANY_ID,COMMISSION,INSURANCE_START_LIMIT, INSURANCE_END_LIMIT,SPECIAL_DISCOUNT, RELATIVE_USER_ID,AMEND_ID,INCEPTION_DATE,ENTRY_DATE,STATUS,MIN_PREMIUM_AMOUNT,BACK_DATE_ALLOWED, LOADING_OF_PREMIUM,PROVISION_FOR_PREMIUM, DISCOUNT_OF_PREMIUM,FREIGHT_DEBIT_OPTION, PRO_COMMISSION FROM   LOGIN_USER_DETAILS WHERE   LOGIN_ID = ? AND USER_ID = LOGIN_USER_DETAILS.user_id";
					String[] args1=new String [4];
					args1[0]=firstName;
					args1[1]=ucode;
					args1[2]=customerId;
					args1[3]=getMasterCustomerLoginId(oacode);
					runner.multipleInsertion(query, args1);
					
					String sql1 = "insert into login_master(LOGIN_ID,PASSWORD,USERTYPE,USERNAME,USERID,AGENCY_CODE,OA_CODE,ACCESSTYPE,RIGHTS,LPASS1,LPASS2,LPASS3,PASSDATE,COMPANY_ID,CREATED_BY,STATUS) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
					String argss[] = new String[16];
					argss[0] = "NONE";
					argss[1] = "";
					argss[2] = "User";
					argss[3] = firstName;
		...
		try 
		{
			...
			try
			{
				try
				{
					...
					args[0] = ucode;
					userLoginId = runner.singleSelection("select isnull(login_id,' ') from login_master where agency_code=?",args);
				...
				if (userType.equals("3")) 
				{
					sqlcheck = "select product_id,isnull(scheme_id,'0') from login_user_details where AGENCY_CODE=?";
				}
				else 
				{
					sqlcheck = "select product_id,isnull(scheme_id,'0') from LOGIN_EXECUTIVE_DETAILS where AGENCY_CODE=?";
				}
				try
				{
	...
	public boolean changePasswordWithDate(String brokerId, String password) 
	{
		...
		try 
		{
			...
			if (qry1.length > 0) 
			{
				sql = "update login_master set  lpass5=?, lpass4=?, lpass3=?,lpass2=?,lpass1=?,password=?,passdate=getdate()+45 ,status='Y' where agency_code=?";
	...
	{
		String[][] countries = new String[0][0];
		String sql = "select COUNTRY_ID,NATIONALITY_NAME from COUNTRY_MASTER where amend_id+country_id in( select max(amend_id)+country_id from country_master group by country_id) and NATIONALITY_NAME is not null order by NATIONALITY_NAME ";
		try 
		{
			countries = runner.multipleSelection(sql);
		}
	...
	public String getMaxCustomerId(String branch)
	{
		...
		try 
		{
			...
			current_no=runner.singleSelection(sql);*/
	...
	public boolean chkpas(String pas, String pass, String brahma)throws Exception 
	{
		...
		String encpass = pass1.crypt(pass.substring(0, 3), pass);
